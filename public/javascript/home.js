...
function saveEdit(rowId, userId) {
    ...
    $.ajax({
        type: 'post',
        url: 'http://localhost/freeform/users/editUser',
        data: {
            edit_row: 'edit_row',
            id: userId,
        ...
        success: function (response) {
            if (response['result']) {
                nodeList.firstNameCell.innerHTML = "<span id='first_name_span" + rowId + "'>" + response['firstName'] + "</span>";
                nodeList.lastNameCell.innerHTML = "<span id='last_name_span" + rowId + "'>" + response['lastName'] + "</span>";
                nodeList.emailCell.innerHTML = "<span id='email_span" + rowId + "'>" + response['email'] + "</span>";
                nodeList.editButton.style.display = "inline-block";
                nodeList.deleteButton.style.display = "inline-block";
                nodeList.saveButton.style.display = "none";
                nodeList.cancelButton.style.display = "none";
                ...
                if (response['firstNameError'].length > 0) {
                    nodeList.firstNameInput.classList.add('table__input-error');
                    nodeList.firstNameError.innerHTML = response['firstNameError'];
                    nodeList.firstNameCell.appendChild(nodeList.firstNameError);
                }else{
                    nodeList.firstNameInput.classList.remove('table__input-error');
                    nodeList.firstNameError.innerHTML = ""
                }
                if (response['lastNameError'].length > 0) {
                    nodeList.lastNameInput.classList.add('table__input-error');
                    nodeList.lastNameError.innerHTML = response['lastNameError'];
                    nodeList.lastNameCell.appendChild(nodeList.lastNameError);

                }else{
                    nodeList.lastNameInput.classList.remove('table__input-error');
                    nodeList.lastNameError.innerHTML = ""
                }
                if (response['emailError'].length > 0) {
                    nodeList.emailInput.classList.add('table__input-error');
                    nodeList.emailError.innerHTML = response['emailError'];
                    nodeList.emailCell.appendChild(nodeList.emailError);
                }else{
                    nodeList.emailInput.classList.remove('table__input-error');
                    nodeList.emailError.innerHTML =""
                }
            }
        },
        beforeSend: function(){
...
function deleteUser() {
    ...
    $.ajax({
        type: 'post',
        url: 'http://localhost/freeform/users/deleteUser',
        data: {
